...
idx_station_base = meta_base['stations'].index(f'{network}.{station}')
idx_component_base = meta_base['components'].index(component)


# %%

station_slice = slice(idx_station_base, idx_station_base+1)
component_slice = slice(idx_component_base, idx_component_base+1)

Nsta = len(meta_base['stations'])
station_slice = slice(0, Nsta)
component_slice = slice(0, 3)


gf_array = gf_array_base[station_slice, component_slice, :].copy()
meta = deepcopy(meta_base)
meta['stations'] = meta['stations'][station_slice]
meta['latitudes'] = meta['latitudes'][station_slice]
meta['longitudes'] = meta['longitudes'][station_slice]
meta['components'] = meta['components'][component_slice]

# Get trace index for subsequent comparison
idx_station = meta['stations'].index(f'{network}.{station}')
idx_component = meta['components'].index(component)
...
def process_homemade(gf_array, meta, starttime, length_in_s, sps, step=True):
    ...
    idata = osl.filter.bandpass(idata, freqmin=0.004, freqmax=1/17.0,
    ...
    if step:
        ...
        print(idata.shape)
        idata = osl.convolve(idata, error.f, 1/sps, tshift)
        print(idata.shape)

    new_meta = deepcopy(meta)
    new_meta['starttime'] = starttime
    new_meta['delta'] = 1/sps
    new_meta['npts'] = int((length_in_s + tshift) * sps)
...
data, pmeta = process_homemade(gf_array, meta, starttime, length_in_s, sps, step=step)
...
plt.plot(gf[0].times(), gf_array[idx_station, idx_component, :], ":", label='Homemade',    lw=lw)
plt.xlim(0, 10800)
plt.legend(frameon=False)
plt.subplot(212)
plt.plot(st[0].times(), st[0].data, label='Obspy', lw=lw)
plt.plot(st[0].times(), data[idx_station, idx_component, :], ':', label='Homemade', lw=lw)
plt.xlim(0, 10800)
plt.legend(frameon=False)
