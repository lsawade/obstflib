...
gcmt_file = os.path.join(scardec_stf_dir, 'CMTSOLUTION')
...
cmt3_file = os.path.join(scardec_stf_dir, f'{gcmt.eventname}_CMT3D+')
...
cmt3_ds_base.compute_geometry(gcmt.latitude, gcmt.longitude)
...
if not os.path.exists(os.path.join(scardec_stf_dir, 'preprocessed', 'waveforms.mseed')):
    ...
    if not os.path.exists(os.path.join(scardec_stf_dir, 'data')):
        ...
        networks = ",".join({_sta.split('.')[0] for _sta in gcmt_ds_base.meta.stations.codes})
        stations = ",".join({_sta.split('.')[1] for _sta in gcmt_ds_base.meta.stations.codes})

        # Run download function
        opl.download_data(
            os.path.join(scardec_stf_dir, 'data'),
            starttime=gcmt.origin_time - 300,
            endtime=gcmt.origin_time + 3600 + 300,
...
gstf.plot(normalize=False, label='GCMT')
...
plt.savefig('peru_data_gcmt_cmt3d_stf.pdf', dpi=300)
...
for phase in phases:
    ...
    onp.tt.get_arrivals(gcmt, ds_cmt3, phase=phase)
...
_, ds_cmt3_tt = ds_gcmt_tt.intersection(ds_cmt3)
...
#%%
fig = plt.figure()
osl.plot.plot_check_section([ds_obsd_tt, ds_gcmt_tt, ds_cmt3_tt], labels=['Observed', 'GCMT', 'CMT3D+'],
                   outfile='atestsection_traveltime_select.pdf')
plt.subplots_adjust(left=0.2, right=0.975, top=0.975, bottom=0.1)
fig.set_size_inches(8, 6)
fig.savefig('peru_data_gcmt_cmt3d_tt_select.pdf')
plt.close(fig)
...
# %%
fig = plt.figure()
osl.plot.plot_check_section([tds_obsd_tt, tds_gcmt_tt, tds_cmt3_tt], labels=['Observed', 'GCMT', 'CMT3D+'],
                   scale=5.0, limits=[5*60,30*60], plot_misfit_reduction=True,)
plt.subplots_adjust(left=0.2, right=0.9, top=0.975, bottom=0.1)
fig.set_size_inches(8, 10)
fig.savefig('peru_data_gcmt_cmt3d_tt_taper.pdf')
plt.close(fig)
...
fobsd, fscar, idx = onp.utils.remove_misfits(tds_obsd_tt, tds_cmt3_tt)
...
ax = osl.plot.plot_check_section([fobsd, fgcmt, fscar], labels=['Observed', 'GCMT', 'CMT3D+'],
...
fig.set_size_inches(8, 10)
fig.savefig('peru_data_gcmt_cmt3d_tt_taper_removed_outliers.pdf')
plt.close(fig)

# %%
misfit_gcmt = onp.utils.L2(fobsd, fgcmt)
misfit_cmt3 = onp.utils.L2(fobsd, fscar)
